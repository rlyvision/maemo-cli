...
TMP="/tmp/maemo"
STOCK_FW_FILE="$TMP/VRZ_XT875_6.7.246.XT875.Verizon.en.US_CFC_01.xml.zip"
STOCK_FW_DIR="$TMP/VRZ_XT875_6.7.246.XT875.Verizon.en.US_CFC_01.xml"
IMAGE="$TMP/maemo-leste-5.0-armhf-droid3-20251228.img.xz"
KEXECDIR="$TMP/bionic-clown-boot"

STOCK_FW_LINK="https://firmware.center/firmware/Motorola/Android/XT875%20Droid%20Bionic%20%28Etna%29/Flash%20Files/Stock/VRZ_XT875_6.7.246.XT875.Verizon.en.US_CFC_01.xml.zip"
IMAGE_LINK="https://maedevu.maemo.org/images/bionic/20260219-daedalus/maemo-leste-5.0-armhf-bionic-20260219.img.xz"
...
read -rp "Is your Droid Bionic on Android 4.04 or older? (yes/no) " yn
[[ "$yn" == "yes" ]] || {
    echo "Follow the manual guide in https://maedevu.maemo.org/images/bionic/README.txt."
    exit 1
}
...
if [[ ! -f "$STOCK_FW_FILE" ]]; then
    wget -Oc "$STOCK_FW_FILE" "$STOCK_FW_LINK"
    mkdir -p "$STOCK_FW_DIR"
    unzip "$STOCK_FW_FILE" -d "$STOCK_FW_DIR"
fi

if [[ ! -d "$KEXECDIR" ]]; then
    git clone https://github.com/IMbackK/bionic-clown-boot "$KEXECDIR"
fi
...
fastboot flash mbm "$KEXECDIR/allow-mbmloader-flashing-mbm.bin"
